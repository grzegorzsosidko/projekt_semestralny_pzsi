            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.news.update', $post) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <hr>
                        <h5>Zmień zdjęcia (opcjonalnie)</h5>
                        <p class="text-muted">Aby zamienić zdjęcie, po prostu wgraj nowe w odpowiednim polu.</p>
                        <div class="mb-3"><label class="form-label">Zmień okładkę</label><input type="file"
                                name="cover_image" class="form-control" accept="image/*"><img
                                src="{{ asset('storage/' . $post->cover_image_path) }}" class="image-preview"
                                style="display:block;"></div>
                        <div class="mb-3"><label class="form-label">Zmień zdjęcie w treści nr 1</label><input
                                type="file" name="image_1" class="form-control" accept="image/*">
                            @if ($post->image_1_path)
                                <img src="{{ asset('storage/' . $post->image_1_path) }}" class="image-preview"
                                    style="display:block;">
                            @endif
                        </div>
                        <div class="mb-3"><label class="form-label">Zmień zdjęcie w treści nr 2</label><input
                                type="file" name="image_2" class="form-control" accept="image/*">
